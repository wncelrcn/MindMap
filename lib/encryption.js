...
// Load environment variables in Node.js environments
if (typeof window === "undefined") {
  try {
    // Try to load .env.local for local development
    require("dotenv").config({ path: ".env.local" });
  } catch (error) {
    // dotenv might not be installed, that's okay for production
  }
}

// Only load secrets on server-side and when needed
const getJWTSecret = () => {
  if (typeof window !== "undefined") {
    throw new Error("JWT operations are not allowed on the client side");
  }
  return process.env.JWT_SECRET || "super-secret";
};
...
const getEncryptionKey = () => {
  ...
  // For local development, generate a temporary key if none exists
  if (!encryptionKey) {
    console.warn("⚠️  ENCRYPTION_KEY not found in environment variables");
    console.warn("⚠️  Using temporary key for local development");
    console.warn(
      "⚠️  Please add ENCRYPTION_KEY to your .env.local file for production"
    );

    // Generate a consistent temporary key for local development
    encryptionKey = crypto
      .createHash("sha256")
      .update("mindmap-local-dev-key")
      .digest("hex");
  }
...
export function encryptData(text) {
  ...
  try {
    ...
    throw error; // Re-throw instead of returning original text for security
  }
}
  ...
  sensitiveFields.forEach((field) => {
    if (decryptedEntry[field] && isEncryptedValue(decryptedEntry[field])) {
      try {
        ...
        throw error; // Re-throw for security - don't return potentially corrupted data
      }
    }
  });
