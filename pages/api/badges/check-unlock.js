...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Update user stats using RPC with timeout and retry
    console.log('Calling update_user_stats for user:', userUID);
    try {
      const { error: updateError } = await Promise.race([
        supabase.rpc('update_user_stats', { p_user_uid: userUID }),
        ...
        switch (badge.badge_type) {
          ...
          case 'theme_specific':
            if (badge.required_themes && badge.required_themes.length > 0) {
              ...
              debugMessage = 'Theme specific check: No required themes specified';
            }
            break;

          case 'theme_variety':
            const uniqueThemes = Object.keys(userStats.theme_counts || {}).length;
          ...
          case 'theme_complete':
            if (badge.required_themes) {
                ...
                try {
                  ...
                  if (categories && categories.length >= 2) {
                    const completedCategories = categories.filter(cat => 
                      (userStats.category_counts?.[cat.id] || 0) > 0
                    ).length;
              ...
              debugMessage = 'Theme complete check: No required themes specified';
            }
            break;
            ...
            if (badge.name === 'Inner Voyager') {
              ...
              if (userStats.longest_entry_words >= 500) {
                ...
                try {
                  ...
                  console.error('Error in Inner Voyager fallback check:', error);
                ...
                if (recentEntries && recentEntries.length >= 3) {
                  ...
                  for (let i = 0; i <= sortedDates.length - 3; i++) {
                    ...
                    if (dayDiff1 === 1 && dayDiff2 === 1) {
                      ...
                      const themes2 = Array.from(dateGroups[sortedDates[i + 1]]);
                      const themes3 = Array.from(dateGroups[sortedDates[i + 2]]);
                      
                      const allThemes = new Set([...themes1, ...themes2, ...themes3]);
                      if (allThemes.size >= 3) {
                  ...
                  debugMessage = `Reflection Star check: Only ${recentEntries?.length || 0} recent entries (need at least 3)`;
                }
              } catch (error) {
